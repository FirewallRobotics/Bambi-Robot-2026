...
public class DriveAssistanceSubsystem extends SubsystemBase {
    ...
    public DriveAssistanceSubsystem(CommandSwerveDrivetrain drivetrain, RobotContainer robotContainer){
    ...
    public void maintainArchAroundHUB(double vx, double vy, CommandXboxController drivercontroller){
        ...
        /**
         * Calculate current position and distance to HUB
         * The distance calculation uses the ratio of Y/X differences
         * based on the alliance color (Blue/Red)
         */
        ...
        /**
         * Calculate distance to HUB using similar triangles:
         * For Blue alliance: (Y difference) / (X difference)
         * For Red alliance: (Y difference) / (X difference)
         */
        switch (DriverStation.getAlliance().get()) {
            case Blue:
                currentdistance = (Constants.VisionSubsystemConstants.BlueHUBCenter[1] - currentpose.getY()) / (Constants.VisionSubsystemConstants.BlueHUBCenter[0] - currentpose.getX());
                break;
            case Red:
                currentdistance = (Constants.VisionSubsystemConstants.RedHUBCenter[1] - currentpose.getY()) / (Constants.VisionSubsystemConstants.RedHUBCenter[0] - currentpose.getX());
                break;
        }

        /**
         * Redirect movement when approaching or moving away from HUB:
         * 1. Calculate direction vector using HUBAngle (in radians)
         * 2. Scale velocity by 0.5 (tunable parameter)
         * 3. Use trigonometry to decompose velocity into X/Y components
         */
