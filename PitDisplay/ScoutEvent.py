...
def scoutMatch(offset, offsetmax, teamnumbers, typematch, match_number, EventName, data):
    global outputmaster
    #print("Formatted: " + str(len(data)) + " matches, and am about to create: " + str(len(teamnumbers)) + " entries")
    print("Thread: " + str(offset) + " to " + str(offsetmax) + " started")
    ...
    for i in range(offset,offsetmax):
        ...
        try:
            ...
            print("Failed to get match data. Error is: " + str(e))
            continue
...
for i in range(0,len(data)):
    ...
    try:
        print("Setting up thread: " + str(i))
        threads.append(threading.Thread(target=scoutMatch, args=(i*6, (i*6)+numberofteams, teamnumbers, typematch, match_number, EventName, data)))
    except:
        print("Failed to create thread: " + str(i))
    #scoutMatch(i*6, (i*6)+numberofteams, teamnumbers, typematch, match_number, EventName, data)

print("All threads created starting")
for i in range(0, len(threads)):
    threads[i].start()
for i in range(0, len(threads)):
    threads[i].join()

#print(str(outputmaster))
...
for i in range(0,len(outputmaster)):
    print("Sending row: " + str(i))
    worksheet.insert_row(outputmaster[i], inputCol)
    time.sleep(2)